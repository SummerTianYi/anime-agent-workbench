...
class DocumentedLimitationTests(unittest.TestCase):
    ...
    def test_single_char_members_cross_match_into_other_classes(self):
        ...
        不是一个已发生的故障。本轮不修它，三条理由：
          ...
          3. 有实测代价，但代价不在颜色类。实测（钉在 test_ranker_mutations 的
             N1SingleCharMemberTests）：删掉颜色类全部 15 个单字，三集命中数一个不
             变、v2 #29「藏青色」仍命中——因为 _masked_scan 最长优先贪心取到长词
             「藏青」而非单字「青」，所以「收紧会伤到 D11」的旧说法不成立。真正的
             代价在别处：删掉全词典 28 个单字 member（颜色15/生日4/宠物4/过敏3/
             称呼2）会丢 v1 #2/#3 与 v2 #24——那些事实没有 head 词，全靠单字 member
             桥接。故 N1 的不修理由不是「收紧会伤到 D11」，而是「单字 member 在部分
             类里承担不可替代的桥接，一刀切删除的代价（丢 3 对）大于收益（消除给干扰
             项加分的噪声），且无分词器时无法按上下文区分『银』作为颜色与『银行』里的
             『银』」。
