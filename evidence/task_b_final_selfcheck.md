...
P18（40 位十六进制串）共 **133** 处，全部是带标签的锚点：blob 哈希、tree 哈希、
...
| 7 | 23 | **0** | 133 | 18 | **0** | 与第 6 轮除时间戳外逐字相同，**提交前的最终不动点** |
...
这七轮里 **P18 与 P19 从未变过**（133 / 18），也就是说修自指泄漏、补记失败经过
这些改动没有引入也没有删除任何一个哈希锚点，只改了正文措辞。
...
> 且两轮退出码均为 0」这个条件的那一轮。**
>
> 本轮实测满足这个条件：上面第 4/5 轮是一对、第 6/7 轮是一对，
> 写完本段之后又跑了一对（共三对），每一对的 `diff`（排除 `generated` 行）均为空、
> 退出码均为 0，且每一对的 P01–P17 均为 0、P18 = 133、P19 = 18 个令牌 / 16 行。
> 入库的就是最后一对里的后一轮。

## 8. `evidence/run_*.json` 计数与 `README.md` 规则 3 的冲突

`ls evidence/run_*.json | wc -l` → **60**。
...
| F13 | 证据口径 | `evidence/README.md` 规则 3「保留最后 10 条」与 `run_all.py` 无裁剪逻辑冲突；当前 60 个，超上限 50 个；且 `run_*.json` 被 gitignore，「保留」对主仓侧不可见，**规则本身无法被执行** | 本文件第 8 节 |
...
6. **H8 同族问题已修**：本轮全部行数由 `wc -l` 生成，计数逐个可复算
   （19 份实质证据文件 + 1 份扫描记录 = 20 份本轮拥有，18 个不含本文件合计
   **3191** 行、60 个 `run_*.json`、39 条 commit）。
...
7. **手工扫描已做到不动点**：19 个探针× 23 个目标文件，泄漏类（P01–P17）
